...
window.onload = function(){ //serve per caricare il contenuto della pagina(supporta il browser)
    aggiornaPost();
}
...
function onJson(json)
{
  ...
  for(const elementi of json){
    ...
    result_box.classList.add('result_box'); //gli aggiungo la classe result_box(tutto il box) a result_box

    const img_box= document.createElement('div'); //inserisci l'url a questa immagine
    img_box.classList.add('img_box');

    const name_box= document.createElement('div');//prendi l'username
    name_box.classList.add('name_box');

    const text_box= document.createElement('div'); //elemento block
    text_box.classList.add('text_box');

    const img = document.createElement('img');
    img.src = elementi.url;

    const name= document.createElement('span'); //elemento inline
    ...
    data.classList.add('data');  //aggiunge la classe del css
